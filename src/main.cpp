    ...
    void start() override {
        ...
        for (int i = 0; i < 110; i++) {
            for (int j = 0; j < 26; j++) {
                ...
                sprite.get_component<air::C_Sprite>().set_size({ 16, 16 });
                sprite.get_component<air::C_Transform>().set_position({ 17 * i, 17 * j });
            }
        }
        ...
        for (int i = 0; i < 110; i++) {
            for (int j = 0; j < 26; j++) {
                ...
                sprite.get_component<air::C_Sprite>().set_size({ 16, 16 });
                sprite.get_component<air::C_Transform>().set_position({ 17 * i, 17 * 26 + 17 * j });
            }
        }
    };
};
